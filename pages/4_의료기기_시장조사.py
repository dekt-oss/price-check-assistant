...
if not service_key:
    st.warning(
        "DATA_GO_KR_SERVICE_KEY가 설정되지 않아 식약처/나라장터 live 조회가 비활성화되어 있습니다. "
        "배포환경에서는 서비스키를 Streamlit Secrets 또는 환경변수에만 저장해야 합니다."
    )
    st.stop()
    ...
    if product_name.strip():
        ...
        if active_models:
            ...
            model_rows = [
                {
                    "모델 일치": (
                        "EXACT" if exact_model and (item.model_name or "").strip().casefold() == exact_model else ""
                    ),
            ...
            st.warning("현재 조회조건에서 국내 공급 후보로 볼 수 있는 동일 품목 등록모델이 0건입니다.")
        ...
        if gate.enabled:
            ...
            if fallback_submitted:
                ...
                if links:
                    st.caption(
                        "동일 품목 공식 후보가 0건일 때만 여는 보조 경로입니다. 웹 결과는 단순 조사 후보이며 "
                        "식약처 품목/허가를 다시 확인하기 전까지 대체장비로 확정하지 않습니다."
                    )
                    ...
                    inactive_rows = [
                        {
                            ...
                            "취소/취하일": (
                                item.cancellation_date.isoformat() if item.cancellation_date else ""
                            ),
                            "수출전용": (
