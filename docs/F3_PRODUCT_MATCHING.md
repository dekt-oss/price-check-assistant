...
| 등급 | 자동판정 기준 | 가격분석 사용 |
| --- | --- | --- |
| A | 정규화된 모델 exact + 제조사 일치/검증 alias + 요구 규격 증거 충족 | 직접가격 |
...
- 검색 규격이 존재하는데 후보에서 모두 확인되지 않으면 B.
...
2. 가격 EvidenceType을 F1 parser로 확정한다.
3. G2B title에서 제품 identity를 파싱한다.
4. F3 matcher가 A/B/C/D/X를 부여한다.
5. A/B이면서 직접가격 EvidenceType인 경우에만 가격범위 계산 대상이 된다.
