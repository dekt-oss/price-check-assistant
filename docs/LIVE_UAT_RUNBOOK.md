...
애플리케이션과 live workflow의 서비스키 우선순위는 다음과 같다.

1. source-specific key
   - G2B: `G2B_SERVICE_KEY`
   - MFDS: `MFDS_SERVICE_KEY`
2. 공통 시장조사 key: `DATA_GO_KR_MARKET_SERVICE_KEY`
3. 기존 하위호환 key: `DATA_GO_KR_SERVICE_KEY`

여러 승인 API가 동일한 data.go.kr 키를 사용하는 환경에서는 `DATA_GO_KR_MARKET_SERVICE_KEY` 하나를 공통 fallback으로 사용할 수 있다. source-specific key가 별도로 있으면 그 값이 우선한다.

Streamlit Community Cloud App Secrets와 GitHub Actions Repository Secrets는 별도 저장소이므로 각 실행환경은 자기 secret store의 값을 사용한다. secret 값 자체는 로그·문서·artifact에 출력하지 않는다.

일부 오래된 CLI가 `DATA_GO_KR_SERVICE_KEY`를 직접 읽는 동안 live workflow는 선택된 G2B/MFDS 키를 workflow-local compatibility alias로만 전달한다. 저장소에 key 값을 기록하지 않는다.
