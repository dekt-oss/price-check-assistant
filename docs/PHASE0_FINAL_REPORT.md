...
Phase 0 transport/finalization 구현은 `cfe46d0365c157588dea870cde9f0b8f66c73e89`에서 확정됐다. 이후 fail-closed 회귀테스트 보강 #21이 추가된 closeout 직전 main 기준선은 다음과 같다.

- main SHA: `faf8ebeef1978af9572d7beaf3714dc5d173b138`
- main CI run: `33821214962`
- 결과: success
- Ruff: pass
- pytest: **108 passed**
...
주의: direct positive가 1건뿐이므로 이 100%는 통계적 일반화 성능이 아니라 **현재 회귀테스트 계약이 정답셋과 일치한다는 의미**다. 테스트 역시 positive 수를 고정값으로 가정하지 않도록 변경했다.
...
현재 실제 live 데이터와 direct price positive를 확보했다. 세부품명 mapping registry를 확대하고, exact model search·pagination·retry·raw evidence 보존을 강화한다. 대량조회에서는 `totalCount` 기반 adaptive date partitioning을 추가한다.
