...
def _write_summary(
    ...
    *,
    ...
    source_summaries = summarize_phase0_by_source(rows, benchmark_products=benchmark_products)
    payload = {
...
def _retained_evidence_reason(candidate_prices: tuple[CollectedPrice, ...]) -> str | None:
    if not candidate_prices:
        return "source searched successfully but no model/manufacturer candidate evidence was retained"
    usable_grades = {MatchGrade.A, MatchGrade.B, MatchGrade.C, MatchGrade.D}
    if not any(price.match_grade in usable_grades for price in candidate_prices):
...
def main(argv: list[str] | None = None) -> int:
    ...
    manufacturer_catalog_error: Exception | None = None
    try:
        manufacturer_catalog = load_manufacturer_public_prices()
    except Exception as exc:
        manufacturer_catalog = ()
        manufacturer_catalog_error = exc
    ...
    for query in queries.values():
        ...
        g2b_mapping = resolve_verified_g2b_mapping(query, g2b_mappings)
        g2b_mapping_status = _mapping_status_for_query(query, g2b_mappings)
        if g2b_mapping is None:
            rows.append(
                build_source_evaluation(
        ...
        if manufacturer_catalog_error is not None:
            rows.append(
                build_source_evaluation(
                    benchmark_model=query.model_name,
                    product_name=query.product_name,
                    ...
                    evaluation_status="error",
                    reason=(
                        f"{type(manufacturer_catalog_error).__name__}: "
                        f"{_safe_error_detail(manufacturer_catalog_error)}"
                    ),
                )
            )
            continue
    ...
    source_summaries = summarize_phase0_by_source(row_tuple, benchmark_products=len(queries))
    ...
    for item in source_summaries:
        avg_elapsed = item.average_elapsed_ms if item.average_elapsed_ms is not None else "N/A"
        print(
            "source_summary="
