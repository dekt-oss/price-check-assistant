...
def main() -> None:
    ...
    if not service_key:
        raise SystemExit(
            "G2B_SERVICE_KEY, DATA_GO_KR_MARKET_SERVICE_KEY, or legacy DATA_GO_KR_SERVICE_KEY "
            "is not configured. Add the key only to an approved secret store for live API calls."
        )
