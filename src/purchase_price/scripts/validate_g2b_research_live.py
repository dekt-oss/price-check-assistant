...
def main() -> int:
    ...
    parser.add_argument("--request-budget-per-case", type=int, default=12)
    ...
    results: list[LiveResearchCaseResult] = []
    for case in load_cases(args.cases):
        discovery = discover_unmapped_g2b_candidates(
            ...
            request_budget=args.request_budget_per_case,
        )
        result = summarize_case(case, discovery)
    ...
    print(f"research_hit_rate={report['research_hit_rate']}")
    print(f"model_candidate_hit_rate={report['model_candidate_hit_rate']}")
