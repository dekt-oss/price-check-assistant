...
from purchase_price.storage.r2 import R2RawEvidenceStore, payload_sha256
...
class HistoricalAuditAccumulator:
    """Tolerant historical auditor that quarantines divergent stable identities.

    Production serving keeps the first stable identity and marks later divergent payloads as
    conflicts. The audit mirrors that behavior so known conflicts are counted instead of aborting
    the corpus scan.
    """
    ...
    def __init__(self) -> None:
        ...
        self._item_hashes: dict[object, str] = {}
    ...
    def add_page(self, page: TrackBRawPage) -> None:
        ...
        within_page_conflicts = sum(issue.code == "IDENTITY_CONFLICT" for issue in result.issues)
        self.identity_conflicts += within_page_conflicts
        ...
        for record in result.records:
            ...
            if existing_hash is not None:
                if existing_hash != record.item_sha256:
                    ...
                    self.issue_counts["CROSS_PAGE_IDENTITY_CONFLICT"] += 1
                    self.classifications.setdefault(record.detail_code, Counter())[
                        "conflict_rows"
                    ] += 1
                else:
            ...
            if raw_price is None or str(raw_price).strip() == "":
                ...
                if record.unit_price is None:
                    ...
                    self.price_counts["positive"] += 1
                elif record.unit_price == 0:
                    self.price_counts["zero"] += 1
                else:
                    self.price_counts["negative"] += 1
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        identities["conflict"] = self.identity_conflicts
        return {
            ...
            "price_counts": dict(sorted(self.price_counts.items())),
            "identity_counts": dict(sorted(identities.items())),
            "amount_check_counts": dict(sorted(self.amount_counts.items())),
