...
class G2BClassificationResearchResult:
    ...
    def research_terms(self) -> tuple[str, ...]:
        ...
        These terms do not establish quote identity, verified mapping, spec equivalence, or
        eligibility for direct-price assessment.
        """
...
def build_classification_lookup_requests(product_label: str) -> tuple[ClassificationLookupRequest, ...]:
    """Plan conservative official-class searches without asserting identity.

    The quote label itself is preserved. Chemical typography such as CO₂ is normalized to CO2,
    and a narrow CO2 -> Carbon dioxide wording expansion is added because PPS official English
    detail-class names use the spelled-out chemical name. Parenthetical text is retained only as
    a specification clue and is not promoted into classification identity.
    """
...
def resolve_classification_research(
    ...
    *,
    ...
    """Resolve official PPS detail-class candidates for Research expansion only.

    API candidates are never written to the verified mapping registry and are never converted to
    direct-price evidence. Failures remain failures/partial states rather than being collapsed to
    a normal zero-result state.
    """
    ...
    for request in lookup_requests:
        ...
        except Exception as exc:  # research source isolation is intentional
            errors.append(exc)
            continue
