...
def bid_notice_numbers(record: G2BResearchRecord) -> tuple[str, ...]:
    ...
    Bid/award records normally expose one identifier. Pre-specification responses may expose a
    comma/space-delimited `bidNtceNoList`; only strings that look like public notice identifiers
    are admitted. Records with no explicit identifier remain unlinked research results.
    """

    raw = record.bid_notice_no or ""
    direct = normalize_bid_notice_no(raw)
    if record.source_type in {G2BResearchSource.BID_NOTICE, G2BResearchSource.AWARD}:
...
def link_procurement_cases(
    ...
    """Group explicit bid/award/pre-spec links by bid notice number.

    This does not infer a link from title similarity, institution or price. Such fuzzy relations
    can be useful for discovery but are not strong enough to claim that an award belongs to a bid.
    """
