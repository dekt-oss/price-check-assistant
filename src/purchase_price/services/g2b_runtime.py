...
def build_configured_g2b_collector(settings: Settings) -> G2BShoppingCollector:
    ...
    default client. Source-specific `G2B_SERVICE_KEY` takes precedence while the original common
    data.go.kr key remains a backward-compatible fallback.
    """

    service_key = settings.resolved_g2b_service_key
    if not service_key:
        raise ValueError("G2B_SERVICE_KEY or legacy DATA_GO_KR_SERVICE_KEY is not configured")
