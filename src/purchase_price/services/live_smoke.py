...
def _failure(*, source_key: str, label: str, started: float, error: Exception, max_http_attempts: int) -> LiveSmokeResult:
    return LiveSmokeResult(source_key, label, LIVE_FAILURE, None, None, _elapsed_ms(started), f"외부 API 요청 실패 ({type(error).__name__}). 0건 조회와 구분됩니다.", logical_requests=1, max_http_attempts=max_http_attempts)


def _success_result(*, source_key: str, label: str, started: float, record_count: int, total_count: int | None, max_http_attempts: int) -> LiveSmokeResult:
    ...
    return LiveSmokeResult(source_key, label, status, record_count, total_count, _elapsed_ms(started), detail, logical_requests=1, max_http_attempts=max_http_attempts)
...
def run_g2b_live_smoke(
    ...
    *,
    ...
    try:
        ...
        return _failure(source_key=source_key, label=label, started=started, error=exc, max_http_attempts=max_http_attempts)
    return _success_result(source_key=source_key, label=label, started=started, record_count=len(page.items), total_count=page.total_count, max_http_attempts=max_http_attempts)
...
def run_mfds_business_live_smoke(company_name: str, *, settings: Settings | None = None, business_client: MfdsBusinessLicenseClient | None = None) -> LiveSmokeResult:
    ...
    try:
        ...
        return _failure(source_key=source_key, label=label, started=started, error=exc, max_http_attempts=max_http_attempts)
    return _success_result(source_key=source_key, label=label, started=started, record_count=len(records), total_count=None, max_http_attempts=max_http_attempts)
