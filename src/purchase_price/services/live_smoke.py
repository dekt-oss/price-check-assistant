    ...
    *,
    ...
    try:
        payload = client.get_json(
            settings.g2b_shopping_base_url or G2B_SHOPPING_BASE_URL,
            G2BShoppingOperation.SPECIFIC_ITEM_PROCUREMENTS.value,
            pageNo=1,
            numOfRows=1,
            inqryDiv=1,
...
def result_from_public_dict(payload: dict[str, Any]) -> LiveSmokeResult:
    return LiveSmokeResult(
        ...
        record_count=(
            None if payload.get("record_count") is None else int(payload["record_count"])
        ),
        total_count=(
            None if payload.get("total_count") is None else int(payload["total_count"])
        ),
