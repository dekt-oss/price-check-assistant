...
def test_adaptive_search_request_budget_fails_closed_before_extra_fetch() -> None:
    collector = AdaptiveStubCollector()
    with pytest.raises(
        g2b_adaptive_search.G2BRequestBudgetExceeded,
...
class MultiYearCollector:
    ...
    def fetch_specific_item_page(self, **kwargs):
        ...
        payload = _payload([], page_no=kwargs["page_no"], total_count=0)
        return g2b_shopping.unwrap_g2b_page(payload), payload


def test_verified_multi_year_search_partitions_before_first_api_call() -> None:
    collector = MultiYearCollector()
    result = g2b_adaptive_search.search_mapped_g2b_candidates_adaptive(
        collector,
        QUERY,
        begin_date=date(2021, 9, 8),
        end_date=date(2026, 9, 5),
        mappings=(VERIFIED_MAPPING,),
    )

    assert len(collector.calls) == 5
    assert all((end - begin).days <= 364 for begin, end in collector.calls)
    assert collector.calls[0][0] == date(2021, 9, 8)
    assert collector.calls[-1][1] == date(2026, 9, 5)
